...
candidates / schools / companies נשמרים כ-JSONB (data column)
כדי לתמוך בכל שדות מודל Pydantic ללא migration.
"""
...
def _wrap(data: Dict) -> Dict:
    """Pack full dict into {id, data} for JSONB tables."""
    return {"id": data["id"], "data": data}


def _unwrap(row: Optional[Dict]) -> Optional[Dict]:
    """Unpack JSONB row back to original dict."""
    if not row:
        return None
...
def upsert_candidate(data: Dict) -> Dict:
    if is_connected():
        result = db_upsert("candidates", _wrap(data))
        return _unwrap(result) or data
...
def get_candidate(candidate_id: str) -> Optional[Dict]:
    if is_connected():
        ...
        return _unwrap(rows[0]) if rows else None
    return _memory["candidates"].get(candidate_id)


def list_candidates(limit: int = 100) -> List[Dict]:
    if is_connected():
        rows = db_select("candidates", limit=limit)
        return [_unwrap(r) for r in rows if r]
...
def upsert_school(data: Dict) -> Dict:
    if is_connected():
        result = db_upsert("schools", _wrap(data))
        return _unwrap(result) or data
...
def list_schools(limit: int = 100) -> List[Dict]:
    if is_connected():
        rows = db_select("schools", limit=limit)
        return [_unwrap(r) for r in rows if r]
...
def upsert_company(data: Dict) -> Dict:
    if is_connected():
        result = db_upsert("companies", _wrap(data))
        return _unwrap(result) or data
...
def list_companies(limit: int = 100) -> List[Dict]:
    if is_connected():
        rows = db_select("companies", limit=limit)
        return [_unwrap(r) for r in rows if r]
...
def list_all_placements_with_feedback(limit: int = 200) -> List[Dict]:
    ...
    school_by_pid    = {fb.get("placement_id"): fb for fb in all_school_fb}
    company_by_pid   = {fb.get("placement_id"): fb for fb in all_company_fb}
    candidate_by_pid = {fb.get("placement_id"): fb for fb in all_candidate_fb}
